...
cols <- as.character(read.xlsx("imp_vars.xlsx", sheetName="Sheet1")[, ])
...
#------------------------------
#Convert character/numeric vars
...
#    length(unique(x)) <= 10)])
#df_temp <- df_cust.clean[, cols] 
#Has captured ANY_MAIL_x and RESPONSEx flags, Converts to factor
...
###############################
#------------------------------
#Prep:16th Campaign observations
#------------------------------
#Subset data frame
...
#NA's, zero's and blanks?
sum(is.na(df_cust.16_imp)) #470867
sum(df_cust.16_imp == 0) #None
sum(df_cust.16_imp == "") #None
...
for (c in cols) {
  ...
  if (p01 > min | p99 < max) {
    nm <- paste(c, "Trim", sep="_")
    df_cust.16_trim[, nm] <- df_cust.16_trim[, c]
    Trim <- quantile(df_cust.16_trim[, c], c(0.01, 0.99))
    df_cust.16_trim[, nm] <- squish(df_cust.16_trim[, nm], Trim)
    #df_cust.16_trim[, c] <- NULL
  }
}

rm(cols, min, max, p01, p99, nm, Trim)
...
#dt_cust.16_imp <- data.table(df_cust.16_imp, key="ACCTNO") 
...
png(filename="var_imp_response16.png", width=1000, height=600, res=150)
...
# Model 1: Naive Bayes
#------------------------------
#Fit model: Naive Bayes: nb 
#Classifier: nbDiscrete
set.seed(1)
...
#------------------------------
#Fit model: Random Forest: ranger
...
fit_glmnet <- train((as.factor(resp_RESPONSE16.train)) ~ .,
                    data=df_cust.16_trainsub,
                    method="glm", trControl=trctrl,preProcess = c("center", "scale"),tuneLength=10)
...
png(filename="resp_m3_glm_insample_roc.png", width=800, height=800, res=150)
...
png(filename="resp_m3_glm_outsample_roc.png", width=800, height=800, res=150)

plot2

png(filename="resp_m3_glm_sample_roc.png", width=1000, height=600, res=150)
...
png(filename="var_totamt16.png", width=1000, height=600, res=150)
...
png(filename="totamt_m1_lms_insample_pred.png", 
    width=800, height=800, res=150)
...
png(filename="totamt_m1_lms_outsample_pred.png", 
    width=800, height=800, res=150)

plot2

png(filename="totamt_m1_lms_sample_pred.png", width=1000, height=600, res=150)
...
png(filename="totamt_m2_rf_insample_pred.png", width=800, height=800, res=150)
...
png(filename="totamt_m2_rf_outsample_pred.png", width=800, height=800, res=150)

plot2

png(filename="totamt_m2_rf_sample_pred.png",  width=1000, height=600, res=150)

grid.arrange(plot1, plot2, ncol=2)

#------------------------------
#Model 3: XGradient Boost
#------------------------------

#Fit model: XGradient Boosting: xgbLinear
set.seed(1)
objControl <- trainControl(method="cv", number=3, 
                           returnResamp="none", allowParallel=TRUE, verboseIter=TRUE)
...
#------------------------------
#Prep: non-16th Campaign observations
#------------------------------
...
for (c in cols) {
  ...
  if (p01 > min | p99 < max) {
    ...
    Trim <- quantile(df_cust.ne16_trim[, c], c(0.01, 0.99))
    df_cust.ne16_trim[, nm] <- squish(df_cust.ne16_trim[, nm], Trim)
...
#------------------------------
# Final prep: non-16th Campaign observations
#------------------------------
